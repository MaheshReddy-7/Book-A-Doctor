...
export const PatientNotifications = () => {
  ...
  const patientNotifs = notifications.filter(n => n.role === 'patient' && n.userId === user.id);
  const unreadCount = patientNotifs.filter(n => !n.isRead).length;

  const handleMarkAllRead = () => {
    markAllNotificationsAsRead('patient', user.id);
    toast.success("All notifications marked as read.");
  };

  return (
    <div className="patient-notifications animate-fade-in">
      <div className="card-custom p-4 bg-white border border-light">
        <div className="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
          <h3 className="h5 fw-bold text-dark mb-0">Alert Notifications</h3>
          {unreadCount > 0 && (
            <button 
        ...
        {patientNotifs.length === 0 ? (
          <EmptyState 
            title="All caught up!" 
            message="No system updates or booking alerts were found for your profile." 
            icon={FaBell} 
          />
        ) : (
