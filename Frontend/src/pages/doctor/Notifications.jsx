...
export const DoctorNotifications = () => {
  ...
  const doctorNotifs = notifications.filter(n => n.role === 'doctor' && n.userId === user.id);
  const unreadCount = doctorNotifs.filter(n => !n.isRead).length;

  const handleMarkAllRead = () => {
    markAllNotificationsAsRead('doctor', user.id);
    toast.success("All notifications marked as read.");
  };

  return (
    <div className="doctor-notifications animate-fade-in">
      <div className="card-custom p-4 bg-white border border-light">
        <div className="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
          <h3 className="h5 fw-bold text-dark mb-0">System Alerts</h3>
          {unreadCount > 0 && (
            <button 
        ...
        {doctorNotifs.length === 0 ? (
          <EmptyState 
            title="All caught up!" 
            message="No system updates or booking requests alerts were found." 
            icon={FaBell} 
          />
        ) : (
          <div className="d-flex flex-column gap-2">
            {doctorNotifs.map((notif) => (
              <div 
                key={notif.id}
