...
async function runTests() {
  console.log('=== BOOK A DOCTOR BACKEND API VERIFICATION ===\n');

  // Let the server boot up if it hasn't
  await sleep(2000);
  ...
  let doctorId = ''; // Doctor document ID
  let appointmentId = '';
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/appointments`, {
      ...
      body: JSON.stringify({
        ...
        notes: 'Please review recent cardiac history if possible.'
      })
    });
    const json = await res.json();
  ...
  console.log('\n=== ALL API Lifecycle Tests Passed Successfully! ===');
}
